    ...
    placeOrder: function (req, res) {
        ...
        NairaAccount.transaction('Debit', 'transfer', tnx_desc, tnx_amt, req.session.userId, '', 'Pending').then(function (ret) {
            if (ret.status == 'success') {
                User.findOne({id: req.session.userId}).exec(function (err, user) {
                    ...
                    Wallet.generateAddress(user.mnemonic, req.session.hash, passphrase).then(function (addr) {
                        ...
                        Order.create(data).exec(function (err, order) {
                            ...
                            Trading.tradeByOrder(order.id, param('btc_qty'), param('prefered_amount'), addr, req.session.userId, ret.tnx_id).then(function (resp) {
                                // broadcast new order
                                if (resp == 'Broadcast') {
                                    sails.sockets.broadcast('trade', 'add-to-order', {
                                        price: param('prefered_amount'),
                                        btc_qty: param('btc_qty')
                                    });
                                    var trade = { id: order.id, amount_per_btc: order.amount_per_btc, btc_qty: order.btc_amount, fees: order.fees };
                                    return res.json(200, {status: 'success', action: resp, trade: trade });
                ...
                sendMail.sendErrMsg('Could not lock down payment for btc order');
            }
        }).catch(function (err) {
            console.log(err);
        });
